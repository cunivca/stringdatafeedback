        ...
        return super.sources.concat(
            {
                options: async (request) => {
                    if (this.validateSearchTerm(request)) {
                        const suggestions = await this.partner_ares.autocomplete(request);
                        suggestions.forEach((suggestion) => {
                            suggestion.classList = "partner_ares_dropdown_many2one";
                        });
                        return suggestions;
                    }
                    else {
                ...
                optionTemplate: "partner_ares.Many2oneDropdownOption",

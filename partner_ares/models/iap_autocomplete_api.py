...
class IapAutocompleteEnrichAPI(models.AbstractModel):
    ...
    def _contact_iap(self, local_endpoint, action, params, timeout=15):
        ...
        account = self.env['iap.account'].get('partner_ares')
        ...
        base_url = self.env['ir.config_parameter'].sudo().get_param('iap.partner_ares.endpoint', self._DEFAULT_ENDPOINT)
        return iap_tools.iap_jsonrpc(base_url + local_endpoint + '/' + action, params=params, timeout=timeout)

    @api.model
    def _request_partner_ares(self, action, params, timeout=15):
        """ Contact endpoint to get autocomplete data.

        :return tuple: results, error code
        """
        try:
            results = self._contact_iap('/iap/partner_ares', action, params, timeout=timeout)

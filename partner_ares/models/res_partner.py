...
class ResPartner(models.Model):
    ...
    def autocomplete(self, query, timeout=15):
        suggestions, _ = self.env['iap.autocomplete.api']._request_partner_ares('search', {
            'query': query,
        }, timeout=timeout)
        if suggestions:
            results = []
    ...
    def enrich_company(self, company_domain, partner_gid, vat, timeout=15):
        response, error = self.env['iap.autocomplete.api']._request_partner_ares('enrich', {
    ...
    def read_by_vat(self, vat, timeout=15):
        vies_vat_data, _ = self.env['iap.autocomplete.api']._request_partner_ares('search_vat', {
            'vat': vat,
        }, timeout=timeout)
        if vies_vat_data:
    ...
    def _is_synchable(self):
        already_synched = self.env['res.partner.ares.sync'].search([('partner_id', '=', self.id), ('synched', '=', True)])
    ...
    def _update_autocomplete_data(self, vat):
        ...
        if vat and self._is_synchable() and self._is_vat_syncable(vat):
            self.env['res.partner.ares.sync'].sudo().add_to_queue(self.id)
